...
def get_log_prob(predictions, plaintext):
    # predictions: probability vector for one trace
    n_classes = predictions.shape[0]
    keys = np.arange(n_classes, dtype=int)
    # Side channel model attack: Key XOR Plaintext = Intermediate Value
...
def compute_key_rank(predictions, plaintexts, keys):
    ...
    # Shuffle to ensure randomness in rank convergence
